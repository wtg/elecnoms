...
func listNominations(w http.ResponseWriter, r *http.Request) {
	...
	if office != "" {
		...
		if pageNumber != "" {
			...
			rows, err = db.Query("SELECT nomination_id, nomination_rin, nomination_initials, valid, page, office_id, date, number FROM nominations WHERE rcs_id = ? AND office_id = ? AND page = ? AND election_id = "+activeElectionQuery+" ORDER BY number", rcs, office, pageNumber)
		} else {
			rows, err = db.Query("SELECT nomination_id, nomination_rin, nomination_initials, valid, page, office_id, date, number FROM nominations WHERE rcs_id = ? AND office_id = ? AND election_id = "+activeElectionQuery+" ORDER BY number", rcs, office)
		}
	} else {
		rows, err = db.Query("SELECT nomination_id, nomination_rin, nomination_initials, valid, page, office_id, date, number FROM nominations WHERE rcs_id = ? AND election_id = "+activeElectionQuery+" ORDER BY number", rcs)
	}
	if err != nil {
...
func addNominations(w http.ResponseWriter, r *http.Request) {
	...
	for _, nomination := range nominations {
		_, err = tx.Exec("INSERT INTO nominations (rcs_id, office_id, nomination_rin, nomination_initials, page, number, election_id) VALUES (?, ?, ?, ?, ?, ?, "+activeElectionQuery+");", rcs, office, nomination.RIN, strings.ToUpper(nomination.Initials), pageNum, nomination.Number)
		if err != nil {
			log.Printf("unable to query database: %s", err.Error())
			http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
...
func modifyNomination(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.Exec("UPDATE nominations SET nomination_rin = ?, nomination_initials = ?, page = ?, valid = ?, number = ? WHERE nomination_id = ?;", nomination.RIN, strings.ToUpper(nomination.Initials), nomination.Page, nomination.Valid, nomination.Number, nomination.ID)
	if err != nil {
		log.Printf("unable to query database: %s", err.Error())
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
